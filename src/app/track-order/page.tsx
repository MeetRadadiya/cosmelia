...
export default function TrackOrderPage() {
  ...
  const handleTrack = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/track?orderId=${encodeURIComponent(orderId.trim())}&email=${encodeURIComponent(email.trim())}`);
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          setTrackingResult({
            ...
            itemsCount: 2,
            lastUpdate: 'Package departed carrier facility in transit to destination.',
          });
        }, 500);
      }
    } catch {
      ...
      <div className="luxury-container max-w-4xl mx-auto">
        ...
        <div className="text-center space-y-3 mb-10 py-4">
          ...
          <p className="text-xs sm:text-sm text-[#5E6472] font-light max-w-md mx-auto">
            Enter your Order ID and billing Email Address below to view real-time shipment progress.
          </p>
        </div>
          ...
          <form onSubmit={handleTrack} className="space-y-4">
            ...
            {error && (
              <div className="p-3 bg-[#FAEEEE] border border-[#B83A3A]/20 rounded-xs text-xs text-[#B83A3A] flex items-center gap-2">
                <svg className="w-4 h-4 text-[#B83A3A] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
        ...
        {trackingResult && (
          <div className="bg-white border border-[#EAE8E1] rounded-sm p-6 sm:p-8 space-y-8 animate-fadeIn">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-6 border-b border-[#EAE8E1] gap-4">
              <div>
                <span className="text-[11px] uppercase tracking-widest text-[#8C734B] font-semibold">Order Reference</span>
                <h2 className="text-xl font-serif text-[#141416] mt-0.5">#{trackingResult.orderId}</h2>
              </div>
              <div className="flex flex-col sm:text-right text-xs text-[#5E6472] space-y-1">
                <span>Courier: <strong className="text-[#141416] font-medium">{trackingResult.courier}</strong></span>
                <span>Tracking #: <strong className="text-[#141416] font-medium">{trackingResult.trackingNumber}</strong></span>
                <span>Estimated Delivery: <strong className="text-[#8C734B] font-semibold">{trackingResult.estimatedDelivery}</strong></span>
              </div>
            </div>
                    ...
                    <div key={step.id} className="flex flex-col items-center text-center space-y-2">
                      ...
                      >
                        {isCompleted && !isCurrent ? (
                          <svg className="w-6 h-6 text-[#C5A059]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                      ...
                      <div>
                        <h4 className={`text-xs font-semibold ${isCompleted ? 'text-[#141416]' : 'text-[#8B92A2]'}`}>
                          {step.label}
                        </h4>
            ...
            {trackingResult.lastUpdate && (
              <div className="p-4 bg-[#FAF9F6] rounded-xs border border-[#EAE8E1] text-xs text-[#5E6472]">
                <span className="font-semibold text-[#141416]">Latest Carrier Update:</span>{' '}
                {trackingResult.lastUpdate}
              </div>
            )}
          </div>
