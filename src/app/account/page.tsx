...
export default function AccountDashboardPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [orderList, addrList] = await Promise.all([getOrders(), getAddresses().catch(() => [])]);
      if (mounted) {
        setOrders(orderList);
  ...
  const pendingOrders = orders.filter((o) => o.fulfillmentStatus !== "fulfilled" && o.financialStatus !== "refunded");
    ...
    <AccountLayout title="Patron Dashboard" breadcrumbLabel="Account">
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          <StatCard icon="orders" label="Total Orders" value={String(orders.length)} link="/account/orders" />
          <StatCard icon="track" label="In Dispatch" value={String(pendingOrders.length)} link="/account/orders" />
          <StatCard icon="address" label="Saved Addresses" value={String(addresses.length)} link="/account/addresses" />
          <StatCard
            icon="profile"
            label="Spend To Date"
          ...
          <AccountCard>
            ...
            <AccountCardBody>
              {loadingAddresses ? (
                ...
                <p className="text-xs text-[#5E6472] py-4">No saved address yet.</p>
              )}
            </AccountCardBody>
          </AccountCard>
...
function StatCard({ icon, label, value, link }: { icon: string; label: string; value: string; link: string }) {
  return (
    <Link
      href={link}
